...
2. 系统读取剪贴板或让用户粘贴分享文案 / 链接，并展示 `智能解析中`。
3. 后端统一识别内容类型：菜谱或打卡地。
...
- 若剪贴板中存在可识别分享文案或链接，直接带入识别输入并进入智能解析中状态。
- 若剪贴板为空或读取失败，展示粘贴输入框。
...
| 值 | 说明 |
| --- | --- |
| `parsing` | 前端本地状态，后端通常不返回；用于展示智能解析中 |
...
| `place_preview_parsing_shown` | 展示智能解析中状态 |
...
| `recipe_preview_parsing_shown` | 展示智能解析中状态 |
...
- [ ] 新增打卡点第一屏先展示 `点击粘贴分享链接`、`打卡地 / 菜谱灵感` 能力说明和
  `手动填写信息` 入口。
- [ ] 点击主入口后先展示 `智能解析中` 状态、加载动画和当前提取阶段文案。
