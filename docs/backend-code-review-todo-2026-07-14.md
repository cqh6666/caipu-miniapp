...
- **进度更新时间**：2026-07-16 11:43:02 +0800
- **状态**：P0/P1、阶段 3 维护性工作包及阶段 4 中有直接证据的 DATA-005、DB-001、
  OPS-003 仓库内实现、核心契约测试、恢复发布链路和
  开发机门禁已完成；待线上
  密钥轮换、仓库外 sidecar、CI 首次运行、生产工具链及真实备份/权限/回滚演练后关闭
  发布门禁
...
| SEC-003 | 请求日志改为路由模板且不记录 query；上传保持随机 URL 公开资源，目录/非生成文件不可访问并带安全缓存头；生产公网基址和服务层大小/半成品清理均已收口 | 仓库内完成；发布前确认线上 `UPLOAD_PUBLIC_BASE_URL` 为正式 HTTPS 地址 |
...
| OPS-001 | SQLite Online Backup 打包 DB/uploads/校验和/元数据，支持保留期与 rsync；每日备份、每周恢复校验 timer、版本化 release、迁移预演、原子切换和失败二进制回滚已落地 | 生产配置异机目标，完成一次真实副本校验、数据恢复和发布失败回滚演练 |
| OPS-002 | `/livez` 与 `/readyz` 已分离，readiness 检查 DB/迁移/目录并携带 release ID；bootstrap 使用专用用户、只写 data 和 systemd 沙箱 | 生产迁移旧 unit，实测 UID、文件权限、上传/字体/WAL、重启及故障 readiness |
| TEST-001 | P0/P1 安全、并发、迁移、入口以及 admin/auth/profile/kitchen/invite/upload/common 契约均有明确断言；CI 不设脱离风险的统一覆盖率阈值 | 确认 Backend CI 首次实际运行成功 |
...
| OPS-003 | release 注入 release/commit/build time/Go toolchain，健康响应与启动日志暴露构建身份；manifest 固化二进制和迁移集合；健康探测配置化，bootstrap 固化 journald 和 5xx/worker/磁盘/备份/恢复巡检 timer | 仓库策略完成；生产需接入外部告警接收端，并实测 journald、巡检 timer 与 request/release ID 检索 |
| DEP-001 | 开发机使用 Go `1.26.5`，`x/image v0.39.0`；全量测试、竞态、vet、模块校验和 `govulncheck` 通过；新增固定工具链和漏洞扫描的 Backend CI | 确认 CI 首次运行成功；升级生产构建机并核对二进制工具链 |

本表只确认仓库和开发机状态，不代表线上运维项或生产发布验收已完成。
...
写权限，停止上限 15 秒覆盖应用 10 秒 shutdown deadline。生产 UID、字体、沙箱与故障
权限演练尚未执行，因此最后一项保持未勾选。
