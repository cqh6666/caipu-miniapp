...
- **进度更新时间**：2026-07-15 18:40:36 +0800
- **状态**：5 个 P0 的仓库内核心修复已大部分落地，待开发机全量验证与上线前运维收口；
  P0 未全部关闭，仍应优先于原路线图 R10～R15
...
### 1.1 当前实施进度（2026-07-15）

| 工作包 | 仓库内进度 | 仍待完成 |
| --- | --- | --- |
| SEC-001 | JWT 用途/签发方/受众/管理员账号校验、三类密钥隔离、凭据 key version 与轮换工具、配置权限默认收紧已落地 | 评估管理员 Bearer Token；执行线上三类密钥轮换并验证旧令牌失效 |
| SEC-002 | 受限 DNS/Dial、重定向复核、超时和打卡点错误回传已落地 | 补齐脱敏安全日志及完整公网图片/超时/取消/重定向测试 |
| SEC-004 | B 站/小红书 host 标签边界、B 站直连 egress policy、字幕域白名单与 SESSDATA 限制已落地 | 验证仓库外 sidecar 具备等价目的地址策略；完成直连/sidecar 全量回归 |
| DATA-003 | claim token、lease、内容版本、CAS 终态和人工编辑保护已落地 | 评估长任务续租；完成多 worker 竞态验证 |
| DEP-001 | `go.mod` 已声明 Go `1.26.5`，`x/image` 已升级到 `v0.39.0` | 开发机/CI/生产工具链升级；全量测试、竞态、vet、govulncheck、CI 门禁与生产二进制确认 |

当前服务器资源有限，维护者要求跳过本机 Go 测试与构建。本表描述代码进度，不代表已通过发布验收。
