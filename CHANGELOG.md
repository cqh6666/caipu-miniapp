...
- **核心改动**：
  - **E1 详情页返回入场动效**（`pages/index/index.vue`）：
    ...
    - 新增 data `libraryEnterMotionActive` / `libraryEnterMotionTimer` / `libraryReturnMotionPending`、method `triggerLibraryEnterMotion(options)`：支持详情页返回后延迟触发，动画结束后自动清 class。
    - `openRecipeDetail()` 标记 `libraryReturnMotionPending`，`onShow()` 仅在该标记存在时延迟 `96ms` 触发；`watch.activeSection` 切回 `library` 时仍即时触发，覆盖"切到厨房 → 切回美食库"路径；`onUnload` 清 timer 与 pending。
    - SCSS 新增 `.library-shell--entering` + `@keyframes library-shell-return-settle`：移除 `opacity` 动画，改为 transform-only 的 `translateY(16rpx → -2rpx → 0)` · `320ms cubic-bezier(0.18, 0.82, 0.18, 1)`，避免微信 `navigateBack` 缓存页被瞬间压透明后闪一下。同步加 `@media (prefers-reduced-motion: reduce)` 兜底。
...
- **影响范围**：
  - `pages/index/index.vue`：token 增 `--color-accent-terracotta`、template `<template v-if>` 改 `<view v-if>` wrapper、data + method + onShow / onUnload 钩、`openRecipeDetail` pending 标记、SCSS 加 `.library-shell` + `.library-shell--entering` + keyframe + reduced-motion。
...
- **兼容性/风险**：
  ...
  - 详情页返回动效不再改 `opacity`，只做位移归位；这样会比淡入更克制，但能避免返回时整页“一闪而过”。
...
- **验证情况**：
  ...
  - 未运行 HBuilderX 真机预览或微信开发者工具构建；建议后续真机补测：① 详情页返回首页时美食库不闪屏、只做轻位移归位；② 点击 spotlight 先收缩淡出再进入菜单详情；③ 左右滑 spotlight 后不会误触进入详情；④ 固顶菜卡书签色调与陶土橙一致。
- **后续待办（P2 / P3）**：
