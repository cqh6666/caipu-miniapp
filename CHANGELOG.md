...
- 修改时间：2026-04-08 12:42 CST
- 变更背景：极简邀请函版本虽然更克制，但在真机聊天截图里出现了“气质是对的、画面却偏空”的问题，尤其单行厨房名场景下，中部留白较大，缺少一点能传达产品特色的记忆点
- 核心改动：后端动态分享图在保持极简邀请函主结构的基础上，补回三枚更克制的品牌语义标签，承接原先“共享菜谱 / 同步菜单 / 自由切换”那类特色信息，但收口为更统一的视觉语气；同时根据标题实际折行数动态调整说明区和信息面板的纵向位置，让单行标题场景更饱满、双行标题场景也不至于拥挤
- 影响范围：`backend/internal/invite/share_image.go`、`pages/index/index.vue`
- 兼容性/风险：本次仍依赖微信客户端对 `imageUrl` 的缓存刷新与缩略图裁切策略；虽然布局已留出底部安全区，但仍建议在微信真机聊天窗口重新发送一次邀请，确认不同机型上的标题折行和缩略图裁切是否符合预期
- 验证情况：已执行 `cd backend && go test ./...`；已执行 `git diff --check`；已完成分享图布局代码级静态自检，尚未在微信真机聊天窗口重新发送邀请做最终视觉验收
