...
- **修改时间**：2026-07-15 18:40:36 +0800
- **变更背景**：承接 7 月 14 日 Go 后端全量代码审查确认的 5 个 P0 工作包，先完成可在仓库内落地的
  身份认证隔离、凭据轮换、出站请求防护、异步任务一致性和依赖升级；当前服务器资源有限，本轮不在服务器执行
  Go 编译、测试、竞态检查或漏洞扫描，改由开发机继续验证。
- **核心改动**：
  - 用户与管理员 JWT 增加独立 `iss`、`aud`、`token_use` 和管理员账号校验；非 `local` 环境拒绝缺失、弱值、
    默认值或互相复用的 `JWT_SECRET`、`ADMIN_JWT_SECRET`、`CREDENTIALS_SECRET`，配置中心默认权限收紧为 `admin`。
  - 凭据密文新增 key version/keyring，兼容旧密文读取，并新增支持 dry-run、事务重加密和回滚 keyring 的
    `cmd/rotate-credentials` 工具。
  - 新增受限出站 HTTP 客户端，在 DNS 与实际拨号阶段拒绝回环、私网、链路本地、组播和未指定地址，限制重定向、
    HTTPS 降级和各阶段超时；应用到远程图片、B 站直连解析和字幕下载，并改用 DNS 标签边界校验平台域名。
  - 新增迁移 `024_add_recipe_job_fencing.sql`，为自动解析和流程图任务增加 claim token、lease 与内容版本；终态写入
    使用 claim/content version 条件更新，旧 worker 或人工编辑后的过期结果会被丢弃。
  - `go.mod` 声明 Go `1.26.5`，`golang.org/x/image` 升级到 `v0.39.0`，同步整理相关间接依赖。
- **影响范围**：Go 后端配置启动校验、用户/管理员登录令牌、运行时密钥密文、远程图片下载、B 站/小红书链接识别、
  菜谱自动解析与流程图 worker、SQLite 自动迁移及 Go 构建依赖；不修改小程序或 Admin Web 页面代码。
- **兼容性或风险**：非本地环境升级前必须确认三类密钥均独立且不少于 32 个非默认字符；轮换
  `CREDENTIALS_SECRET` 时需临时配置旧 keyring 并先 dry-run。数据库升级会新增任务 fencing 字段。当前仍未执行线上
  三类密钥轮换、sidecar 等价 SSRF 策略验证、管理员 Bearer Token 去留评估、长任务续租、CI 漏洞门禁和生产二进制
  工具链确认，不能把 5 个 P0 视为全部发布关闭。
- **验证情况**：仅完成代码与文档静态审阅；按维护者要求未在当前低配服务器执行 `go test`、`go test -race`、
  `go vet`、`govulncheck` 或构建。当前服务器 `go version` 仍为 `go1.26.1`，需在开发机使用 Go `1.26.5` 或更高兼容
  补丁版本完成全量验证后再发布。

## 2026-07-14 (Go 后端全量代码审查与安全待办重排)

### Changed

- **修改时间**：2026-07-14 22:49:24 +0800
- **变更背景**：R0～R9 已完成结构拆分，原后续路线主要聚焦测试与可维护性；本次对 `backend/` 生产代码、
  配置、迁移、依赖和运维脚本做全量复审，需要根据新发现的安全与一致性风险重新排序下一阶段工作。
- **核心改动**：新增并通过安全、数据一致性、运维三条独立 Agent 审查线复核
  `docs/backend-code-review-todo-2026-07-14.md`。将用户/管理员 JWT 默认共用密钥与公开默认密钥、远程图片 SSRF、
  B 站直连解析 SSRF、自动解析覆盖人工编辑/旧 worker 覆盖新任务，以及 Go 工具链与 `golang.org/x/image` 可达漏洞
  列为 5 个 P0 工作包；补充配置文件覆盖进程环境、默认空间重复、整行更新丢失、成员权限竞态、Admin B 站配置外键
  冲突、AI 上游响应边界/SSE 脱敏、readiness、systemd 最小权限、审计原子性和逐项验收标准。
- **影响范围**：仅新增后端审查与重构待办文档并调整后续实施优先级；不修改 Go 运行时代码、HTTP/JSON 契约、
  数据库、环境变量、部署脚本或当前线上配置。
- **兼容性或风险**：文档中的 5 个 P0 为已确认风险，应优先于旧路线图 R10～R15；本次没有直接修复、轮换线上密钥
  或修改部署配置，在对应工作包关闭前风险仍然存在。小红书伪后缀当前只确认输入校验错误，后端未直接访问目标，
  因此按 sidecar 边界待验证处理。其余 P1/P2 项需按进入条件实施，禁止用机械拆文件替代行为修复。
- **验证情况**：`cd backend && go test ./... -count=1`、`go test -race ./... -count=1`、`go vet ./...`、
  `go mod verify` 和脚本 `bash -n` 均通过，整体语句覆盖率 42.8%；`govulncheck ./...` 命中 12 个可达漏洞。
  多 Agent 复审额外执行 `go test ./internal/... -cover -count=1` 并通过；启用 SQLite 外键的最小复现确认 Admin
  `updated_by=0` 会触发 `FOREIGN KEY constraint failed`。上述失败结果已进入 P0/P1 待办，未验证线上实际构建
  二进制版本与运行配置。

## 2026-07-14 (小程序链接解析能力文案准确化)

### Changed

- **修改时间**：2026-07-14 22:09:45 +0800
- **变更背景**：微信小程序审核反馈当前服务文案涉及“AI 问答、AI 绘画”等深度合成能力，个人主体暂未开放对应类目；
  其中添加菜谱面板将 B 站已有字幕提取能力写成“AI字幕提取”，容易把内容提取误解为生成式服务。
- **核心改动**：B 站能力说明改为“视频字幕提取 / 菜谱整理”；添加链接流程的“智能解析中”统一改为
  “内容解析中”；来源提示“AI 识别”改为“自动识别”；菜谱重新整理确认改用“整理额度”；空间统计改为
  “已完成解析”。同步更新链接解析产品设计与 POC 文档中的对应文案。复审后将“AI 助手入口”前后端默认值
  改为关闭；小程序不再使用旧缓存决定首屏入口，配置缺失、非法、请求中或请求失败均保持关闭，仅接受服务端明确开启。
- **影响范围**：小程序添加菜谱/打卡点面板、菜谱详情重新整理确认、空间统计标签及链接解析说明文档；
  小程序公开功能开关默认策略、后端 `miniapp.features.diet_assistant_enabled` 默认值及对应测试；不修改链接解析算法、
  字幕处理方式、HTTP/JSON 字段、额度规则或数据结构。
- **兼容性或风险**：本次仅使用户文案更贴近实际任务，不改变服务能力。若提审环境仍开启“饮食管家”问答入口，
  审核方仍可能按真实 AI 问答功能判断服务类目，不能依靠文案调整规避主体或类目要求。
- **验证情况**：`npm test`、`cd backend && go test ./... -count=1`、4 个受影响 SFC 脚本/模板编译与
  `git diff --check` 均通过；HBuilderX 5.07 明确输出“项目 caipu-miniapp 编译成功”，微信开发者工具
  `auto-preview` 成功完成。

## 2026-07-14 (前端代码审查问题收口重构)

### Fixed

- **修改时间**：2026-07-14 21:45:14 +0800
- **变更背景**：承接 `docs/frontend-code-review-2026-07-14.md` 对最近两轮前端模块化结果的复审，修复拆分后
  首页成员展示与 session 应用链路中的未定义符号，并继续收口页面层重复的图片缓存、空间切换和智能识别编排。
- **核心改动**：
  - 修复空间成员格式化方法与实际模块函数命名不一致的问题；将 `applySession` 从菜谱库模块移回页面编排层，
    新增 `onKitchenChange` 模块生命周期，由地点、菜单和空间模块各自重置所属状态。
  - 新增通用图片展示缓存 controller，统一首页封面、详情 Hero 和流程图的缓存命中、后台预热、竞态保护、
    本地失败回退远端及远端失败隐藏；菜谱卡片模型不再内嵌缓存封面，避免预热期间反复重建整张列表。
  - 步骤完成 key 改为 computed 缓存；空间统计数字动效改为一批 key 共用计时器并批量写入；统计数据请求并发执行，
    菜单购物车按实际条目查找菜谱。
  - 智能识别叶子组件只负责剪贴板输入与事件上报，请求、空间上下文和阶段编排统一归属 `use-smart-add.js`；
    两个面板的公共 SCSS 收口为单文件。
  - 菜谱图片来源、图片版本、解析内容文本化与动作反馈 controller 上移到 `utils/`；删除两个死状态并复用编辑归一化逻辑。
- **影响范围**：微信小程序首页、菜谱详情、空间统计、添加菜谱/打卡点智能识别及对应 Miniapp 规则测试；
  不修改后端 API、数据库、路由、持久化 key、页面视觉或 Admin 业务代码。
- **兼容性或风险**：保持缓存 key、远端图片回退、菜谱公开只读、解析超时占位菜谱和空间切换数据隔离语义；
  图片远端地址也加载失败时会隐藏对应图片，流程图会自动回到详细步骤。尚未做真机点击回归。
- **验证情况**：`npm test`、6 个受影响 Miniapp SFC 脚本/模板编译与 `git diff --check` 均通过；
  HBuilderX 5.07 明确输出“项目 caipu-miniapp 编译成功”，微信开发者工具 `auto-preview` 成功完成。

## 2026-07-14 (后端重构完成后复审与后续路线补充)

### Changed

- **修改时间**：2026-07-14 08:51:49 +0800
- **变更背景**：R0～R9 后端可维护性重构已完成并推送，需要基于提交后的实际文件体量、包依赖和测试覆盖率继续判断
  后续投入方向，避免把机械拆文件误当成优化，并为下一阶段建立明确的进入条件和验收口径。
- **核心改动**：
  - 复跑后端全量测试、竞态测试、静态检查和覆盖率统计，确认整体覆盖率 42.8%，主要风险集中在
    `admin/auth/invite/kitchen/upload/common/profile` 的业务契约与安全边界测试不足。
  - 更新 `docs/backend-refactor-roadmap-2026-07-14.md`，新增 R10～R15 候选工作，依次覆盖核心契约测试与 CI、
    Admin HTTP 边界、外部 HTTP 适配器、worker 生命周期，以及触发式的读模型/状态仓储和配置分组优化。
  - 更新 `backend/README.md`，明确 R0～R9 已完成、后续路线入口和推荐执行顺序。
- **影响范围**：仅更新后端维护路线、测试策略和文档入口；不修改 Go 生产代码、HTTP/JSON 契约、数据库、环境变量、
  构建产物或部署行为。
- **兼容性或风险**：R10～R15 均为待排期或观察项，不代表当前版本存在已知行为缺陷，也不自动承诺实施；后续必须按
  文档中的进入条件逐项启动。当前不建议引入 DI 框架、ORM 或按行数继续机械拆分。
- **验证情况**：`cd backend && go test ./... -count=1`、`go test -race ./... -count=1`、`go vet ./...`、
  `gofmt -d internal cmd`、`go test ./internal/... -cover -count=1` 与提交前 `git diff --check` 均通过；文档变更后再次执行
  `git diff --check`。
