...
- 修改时间：2026-04-08 11:41 CST
- 变更背景：上一版邀请分享图已经摆脱了页面截图，但在微信聊天卡片里仍然偏“详情页思路”，信息密度过高，且底部信息面板与邀请码条存在真实重叠，缩略图可读性不足
- 核心改动：后端动态分享图改为更偏聊天卡片的结构表达，保留厨房名、邀请人、可加入状态和邀请码，同时把成员数、剩余名额、有效期收口为一行元信息，并新增更明确的“打开卡片即可加入 / 立即加入”动作提示；前端分享标题同步去重，避免和发送者昵称、图内邀请语形成三次重复
