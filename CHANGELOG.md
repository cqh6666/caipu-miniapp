...
- 核心改动：保留规则提取来源链接，模型只参与低置信度标题清洗；标题模型现已支持独立配置地址与密钥
- 影响范围：`pages/index/index.vue`、`backend/internal/linkparse/*`
- 兼容性/风险：当前仍只支持 `bilibili` / `xiaohongshu` 两个平台；若运行环境未配置 AI 模型，标题会完全沿用规则清洗结果
- 验证情况：已执行 `cd backend && go test ./internal/linkparse`
