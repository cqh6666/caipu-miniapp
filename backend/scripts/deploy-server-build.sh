...
ssh "$SERVER_HOST" \
	"REPO_DIR='$REPO_DIR' BACKEND_DIR='$BACKEND_DIR' SERVICE_NAME='$SERVICE_NAME' APP_PORT='$APP_PORT' LIVENESS_PATH='$LIVENESS_PATH' READINESS_PATH='$READINESS_PATH' SYSTEMCTL_BIN='$SYSTEMCTL_BIN' BUILD_NICE='$BUILD_NICE' GO_BUILD_GOMAXPROCS='$GO_BUILD_GOMAXPROCS' PLAN_ONLY='$PLAN_ONLY' bash -s" <<'REMOTE'
set -euo pipefail
