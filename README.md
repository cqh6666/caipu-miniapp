...
- 首页“清单”支持在 `美食库 / 打卡点` 间切换；打卡点已接入后端接口，
  支持按空间同步、本地缓存、新增、编辑、删除、搜索、筛选、标记想去/去过，
  并可通过微信地图选点和打开地图。
...
- 新增邀请页，支持预览邀请并加入共享空间
- 后台配置中心支持控制小程序首页底部中间按钮是否打开 AI 助手；关闭后回到添加菜谱弹层
...
- 后台 `配置中心 -> 小程序功能开关 -> AI 助手入口` 控制首页底部中间按钮：
  - 开启：打开饮食管家 AI 助手
  - 关闭：打开添加菜谱弹层
  - 默认关闭；配置缺失或小程序拉取失败时也保持关闭，只有服务端明确开启后才展示问答入口
  - 小程序通过 `/api/public/app-config` 拉取公开功能开关；每次拉取完成前按关闭处理
- `utils/auth.js` 负责登录态恢复、空间上下文和 token 持久化
- `utils/recipe-store.js` 负责菜品数据归一化、本地缓存和远端同步
- `utils/place-store.js` 负责打卡点数据归一化、图片上传、本地缓存和远端同步
...
- 饮食管家默认直连 LongCat OpenAI-compatible `/chat/completions`，已接入
  function calling：当前支持查看当前空间菜谱数量、按菜名或食材模糊查询菜谱、
  按菜谱 ID 获取菜谱详情，以及根据 B 站 / 小红书链接解析并保存菜谱食材；工具执行阶段会通过 SSE 返回实时状态，
  最终回复继续通过 SSE 流式返回小程序；支持通过
  `DIET_ASSISTANT_AI_THINKING_TYPE` 和 `DIET_ASSISTANT_AI_REASONING_EFFORT`
  配置 DeepSeek-style thinking 请求参数
...
- `随机`：
  从当前可选列表中随机抽取一道菜
- `添加`：
  美食库模式下通过底部中间按钮打开抽屉式表单新增菜品；打卡点模式下打开
  “添加打卡点”表单
- `详情`：
  点击菜谱卡进入菜品详情页；点击打卡点卡片打开详情弹层；点击首页菜单安排卡进入菜单详情页
- `编辑 / 删除`：
  在菜品详情页、打卡点详情弹层或菜单详情页底部固定操作栏中进行
